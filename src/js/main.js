$(document).ready(function () {
    //carusel,слайдер
    const owl = $('#watch');
    owl.owlCarousel({
        items: 1,
        nav: false,
        loop: true,
        dots: false,
        autoHeight: true,
    });
// Go to the next item
    $('.customPrev').click(function () {
        owl.trigger('next.owl.carousel');
    });
// Go to the previous item
    $('.customNext').click(function () {
        owl.trigger('prev.owl.carousel', [300]);
    });

    //tabs
    $('div.catalog-tabs').on('click', 'button:not(.catalog-tabs__tab_active)', function () {
        $(this)
            .addClass('catalog-tabs__tab_active').siblings().removeClass('catalog-tabs__tab_active')
            .closest('div.container').find('div.catalog-content').removeClass('catalog-content_active')
            .eq($(this).index()).addClass('catalog-content_active');

    });
    //click btn
    $('.catalog-content-card__moreBtn').each(function (i) {
        $(this).on('click', function (e) {
            e.preventDefault();
            $('.catalog-content-card__click').eq(i).addClass('catalog-content-card__click_active');
            $('.catalog-content-card__itemMore').eq(i).addClass('catalog-content-card__itemMore_active');
        })
    });
    // back btn
    $('.catalog-content-card-more__backBtn').each(function (i) {
        $(this).on('click', function (e) {
            e.preventDefault();
            $('.catalog-content-card__click').eq(i).removeClass('catalog-content-card__click_active');
            $('.catalog-content-card__itemMore').eq(i).removeClass('catalog-content-card__itemMore_active');
        })
    });
    //скрол
    $(window).scroll(function () {
        if($(this).scrollTop() > 500){
            $('.up-arrow').fadeIn()
        } else{
            $('.up-arrow').fadeOut();
        }
    });
    $('.header-menu-call__btn, .home__btn').on('click',function () {
        $('.overlay, .popup, .consult').fadeIn('slow')
    });
    $('.popup__close,.consult').on('click',function () {
        $('.overlay, .popup, .consult, .thanks, .buy-form').fadeOut('slow')
    });
    $('.consultation-form__btn').on(function () {
        $('.overlay, .popup, .thanks').fadeIn('slow');
        $('.consult').fadeOut()
    });

    $('.catalog-content-button__buybtn').each(function (i) {
        $(this).on('click',function () {
            $('.buy-form .buy-form__subtitle').text($('.catalog-content-card__title').eq(i).text());
            $('.overlay, .popup, .buy-form').fadeIn('slow');
        })
    })
});
//more button
   /* $('.catalog-content-card__moreBtn').on('click', function (e) {
        e.preventDefault();
        $('.catalog-content-card__click').addClass('.catalog-content-card__click_active');
        $('.catalog-content-card__itemMore').addClass('.catalog-content-card__itemMore_active')

    });
    //back btn
    $('.catalog-content-card-more__backBtn').on('click', function (e) {
        e.preventDefault();
        $('.catalog-content-card__click').removeClass('.catalog-content-card__click_active');
        $('.catalog-content-card__itemMore').removeClass('.catalog-content-card__itemMore_active')

    });
*/

//click
/*
function moreBtn() {
    let moreBtn = document.getElementById("itemMore");
    let backBtn = document.getElementById("backBtn");
    moreBtn.style.display = "block";
    backBtn.style.display = "none";
}
function prevBtn(){
    let moreBtn = document.getElementById("itemMore");
    let backBtn = document.getElementById("backBtn");
    moreBtn.style.display = "none";
    backBtn.style.display = "block";

}*/

/*$('div.catalog-content-card').on('click', 'a:not(.catalog-content-card__click_active)', function() {
    $(this)
        .addClass('catalog-tabs__tab_active').siblings().removeClass('catalog-tabs__tab_active')
        .closest('div.container').find('div.catalog-content').removeClass('catalog-content_active')
        .eq($(this).index()).addClass('catalog-content_active');*/

